        ...
        private void SendNotificationsInSeparateThread(EmailNotifier emailNotifier)
        {
            ...
            try
            {
                ...
                mailMessage.From = new MailAddress(
                    SettingUtils.GetStringSetting(SettingUtils.SETTING_EMAIL_FROM),
                    SettingUtils.GetStringSetting(SettingUtils.SETTING_EMAIL_FROM)
                );
                ...
                networkCredentials = new NetworkCredential(
                    SettingUtils.GetStringSetting(SettingUtils.SETTING_EMAIL_USERNAME),
                    SettingUtils.GetStringSetting(SettingUtils.SETTING_EMAIL_PASSWORD)
                );

                // Create the smtp client using SSL only
                smtpClient = new SmtpClient(
                    SettingUtils.GetStringSetting(SettingUtils.SETTING_EMAIL_SMTP),
                    Convert.ToInt32(587)
                );
