...
{
    public class WorkflowRuleMiddleware
    {
        readonly RequestDelegate next;

        public WorkflowRuleMiddleware(RequestDelegate next)
        {
            this.next = next;
        }
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            try
            {
                ...
                mode = BaseHttpUtils.GetModeFromQuery(context.Request.QueryString);

                // Get any provided notification email
                email = BaseHttpUtils.GetEmailFromQuery(context.Request.Query);
                ...
                tenantId = segments[7].Replace("/", "");
                flowId = segments[8].Replace("/", "");
                player = segments[9];
                ...
                receivedNotification.ExtractData(emailNotifier, new StreamReader(context.Request.Body).ReadToEnd(), mode);
                ...
                receivedNotification.PrepareResponse(context);
            }
