...
namespace ManyWho.Service.Salesforce
{
    public class SalesforceServiceSingleton
    {
        ...
        public const String GROUP_SELECTION_ALL_GROUPS = "Groups";
        public const String GROUP_SELECTION_PROFILE = "Profile";
        public const String GROUP_SELECTION_ROLE = "Role";
                        ...
                        objectDataResponseAPI.objectData.Count > 0)
                    {
                        ...
                        foreach (ObjectAPI objectAPI in objectDataResponseAPI.objectData)
                        {
                            ...
                            foreach (PropertyAPI propertyAPI in objectAPI.properties)
                            {
                                ...
                                else if (propertyAPI.developerName.Equals("DeveloperName", StringComparison.InvariantCultureIgnoreCase) == true)
                                {
            ...
            try
            {
                    ...
                    String.IsNullOrWhiteSpace(authenticationCredentialsAPI.code) == false)
                {
                    if (String.IsNullOrWhiteSpace(authenticationCredentialsAPI.code) == false)
                    {
                        ...
                        authenticatedUser.token = this.CreateSalesforceAuthenticationToken(serviceToken, chatterBaseUrl + "/services/Soap/u/45.0");
