...
namespace ManyWho.Service.Salesforce.Utils
{
    public class BaseHttpUtils
    {
        ...
        public static String GetModeFromQuery(QueryString query)
        {
            String mode = null;

            // Check to see if the caller passed in the mode
            if (query.Value.IndexOf(ManyWhoConstants.MODE_DEBUG_STEPTHROUGH, StringComparison.OrdinalIgnoreCase) > 0)
            {
                mode = ManyWhoConstants.MODE_DEBUG_STEPTHROUGH;
            }
            else if (query.Value.IndexOf(ManyWhoConstants.MODE_DEBUG, StringComparison.OrdinalIgnoreCase) > 0)
            {
        ...
        public static String GetEmailFromQuery(IReadableStringCollection query)
        {
