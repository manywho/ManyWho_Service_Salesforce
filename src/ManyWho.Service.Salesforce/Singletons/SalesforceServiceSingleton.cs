...
namespace ManyWho.Service.Salesforce.Singletons
{
    public class SalesforceServiceSingleton
    {
        ...
        /// </summary>
        public async Task<MessageAPI> ShareMessage(IAuthenticatedWho authenticatedWho, String streamId, IFormCollection formCollection)
        {
            WebException webException = null;
            HttpResponseMessage httpResponseMessage = null;
            HttpClient httpClient = null;
            ...
            {
                throw new ArgumentNullException("FormCollection", "FormCollection object cannot be null");
            }
            ...
            {
                if (formKey.Equals(ManyWhoConstants.SERVICE_REQUEST_FORM_POST_KEY, StringComparison.OrdinalIgnoreCase) &&
                    formCollection[formKey].Count > 0)
                {
                    socialServiceRequest = JsonConvert.DeserializeObject<SocialServiceRequestAPI>(formCollection[formKey][0]);
                }
            }
        ...
        /// </summary>
        public async Task<MessageAPI> PostNewMessage(IAuthenticatedWho authenticatedWho, String streamId, IFormCollection formCollection)
        {
            WebException webException = null;
            HttpResponseMessage httpResponseMessage = null;
            HttpClient httpClient = null;
            ...
            for (int i = 0; i < SalesforceHttpUtils.MAXIMUM_RETRIES; i++)
            {
                try
                {
                        ...
                        formCollection.Files.Count > 0)
                    {
                        ...
                        foreach (IFormFile fileData in formCollection.Files)
                        {
                            ...
                            fileName = Path.GetFileName(ContentDispositionHeaderValue.Parse(fileData.ContentDisposition).FileName);
                            ...
                            using (var fileStream = fileData.OpenReadStream())
                            {
                                ...
                                content = new ByteArrayContent(fileBinaryData);
                                ...
                                multipartFormDataContent.Add(content, "feedItemFileUpload", fileName);
        ...
        public async Task<ObjectDataResponseAPI> UploadFile(IAuthenticatedWho authenticatedWho, IFormCollection formCollection)
        {
            ...
            foreach (var formKey in formCollection.Keys)
            {
                if (formKey.Equals(ManyWhoConstants.FILE_DATA_REQUEST_FORM_POST_KEY, StringComparison.OrdinalIgnoreCase) &&
                    formCollection[formKey].Count > 0)
                {
                    fileDataRequest = JsonConvert.DeserializeObject<FileDataRequestAPI>(formCollection[formKey][0]);
                }
            }
            ...
            if (formCollection.Files != null &&
                formCollection.Files.Count > 0)
            {
                sObject[] saveObjects = new sObject[formCollection.Files.Count];
                int counter = 0;

                // Go through all of the file data in the stream provider
                foreach (var fileData in formCollection.Files)
                {
                    String fileName = null;
                    byte[] fileBinaryData = null;
