...
using ManyWho.Service.Salesforce.Controllers;
                ...
                textBody.Trim().Length > 0)
            {
                    ...
                    outcomes.Count > 0)
                {
                    foreach (OutcomeAvailableAPI outcome in outcomes)
                    {
                        ...
                        {
                            textBody += outcome.label + "(Click here: " + SettingUtils.GetStringSetting(SettingUtils.SETTING_SERVER_BASE_PATH) + "/api/email/outcomeresponse?token=" + token + "&selectedOutcomeId=" + outcome.id + "&redirectUri=" + Uri.EscapeUriString(redirectUri) + ")" + Environment.NewLine;
                        }
                    }
                }
                ...
                htmlBody.Trim().Length > 0)
            {
                    ...
                    outcomes.Count > 0)
                {
                    ...
                    foreach (OutcomeAvailableAPI outcome in outcomes)
                    {
                        if (outcome.id.Equals(ManyWhoConstants.FAULT_GUID.ToString(), StringComparison.OrdinalIgnoreCase) == false)
                        {
                            fullHtmlBody += "<a href=\"" + SettingUtils.GetStringSetting(SettingUtils.SETTING_SERVER_BASE_PATH) + "/api/email/outcomeresponse?token=" + token + "&selectedOutcomeId=" + outcome.id + "&redirectUri=" + Uri.EscapeUriString(redirectUri) + "\" class=\"btn btn-primary\">" + outcome.label + "</a>&nbsp;";
                        }
                    }
